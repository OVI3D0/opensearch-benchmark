...
class BenchmarkCoordinator:
    ...
    def setup(self, sources=False):
        ...
        if database_type.lower() != "opensearch":
            # Non-OpenSearch databases don't have OpenSearch distribution versions, and the
            # auto-detect probe below uses OS-shaped /_cluster/health and /_cat/indices
            # routes that either don't exist or differ on Vespa/Milvus. Skip the probe and
            # set a default distribution.version so workload templates still render against
            # the latest (3.x) workload branch.
