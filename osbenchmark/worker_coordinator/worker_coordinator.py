    ...
    def drive(self):
        ...
        if self.at_joinpoint():
            ...
            if self.complete.is_set():
                ...
                self.sampler = metrics_processing.DefaultSampler(start_timestamp=time.perf_counter(), buffer_size=self.sample_queue_size)
                self.profile_sampler = metrics_processing.ProfileMetricsSampler(start_timestamp=time.perf_counter(), buffer_size=self.sample_queue_size)
...
class AsyncIoAdapter:
    ...
    async def run(self):
        ...
        try:
            ...
            # Comprehensive OpenSearch client cleanup to prevent memory leaks
