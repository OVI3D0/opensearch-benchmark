...
class BulkVectorDataSet(Runner):
    ...
    async def __call__(self, opensearch, params):
        ...
        # parse responses lazily in the standard case - responses might be large thus parsing skews results
        if not detailed_results:
            opensearch.return_raw_response()
        ...
        for attempt in range(retries):
            try:
                ...
                # Analyze response to get success/error statistics
                stats = self.detailed_stats(params, response) if detailed_results else self.simple_stats(size, response)
    ...
    def simple_stats(self, size, response):
        bulk_success_count = size
        bulk_error_count = 0
        error_details = set()
        # parse lazily on the fast path
        props = parse(response, ["errors", "took"])
        ...
        if props.get("errors", False):
            # determine success count because we need to iterate through all items anyway
            bulk_success_count = 0
            # Reparse fully in case of errors - this will be slower
            parsed_response = json.loads(response.getvalue())
            for item in parsed_response["items"]:
    ...
    def detailed_stats(self, params, response):
        ...
        # Body is always a list for vector data sets
        bulk_lines = params["body"]

        # Calculate sizes - alternating action/document lines
        for line_number, data in enumerate(bulk_lines):
            line_size = len(json.dumps(data).encode('utf-8')) if isinstance(data, dict) else len(data.encode('utf-8'))
            # Only odd-numbered lines are documents (0-indexed, so line_number % 2 == 1)
            if line_number % 2 == 1:
                total_document_size_bytes += line_size
            bulk_request_size_bytes += line_size
